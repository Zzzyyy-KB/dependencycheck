...
import edu.zju.cst.aces.sootex.callgraph.CallGraphRefiner;
...
//        PhaseOptions.v().setPhaseOption("jb.ulp", "enabled:false");
     ...
     */
    public static void setSootEntryPoints(Collection<String> entryPointsSigs) {
    ...
    public static void doFastSparkPointsToAnalysis(Map<String,String> opt, CGType cgtype, CallGraphRefiner refiner) {
        opt.put("simulate-natives","true");
        opt.put("implicit-entry","true");
        if (cgtype.equals(CGType.GEOM)) {
            SootUtils.doGeomPointsToAnalysis(opt);
        } else if (cgtype.equals(CGType.SPARK)) {
            SootUtils.doSparkPointsToAnalysis(opt);
        } else if (cgtype.equals(CGType.RTA)){
            SootUtils.doRTAPointsToAnalysis(opt);
        } else {
            SootUtils.doVTAPointsToAnalysis(opt);
        }
        if (refiner != null) {
